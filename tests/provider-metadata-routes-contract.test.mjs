...
test('provider metadata routes preserve request identity and visible failures', async () => {
    ...
    const requests = [
        { path: '/api/openai/workers-ai/models/embedding', body: { workers_ai_account_id: 'account-id' }, command: 'get_workers_ai_embedding_models', args: { dto: { workers_ai_account_id: 'account-id' } } },
        { path: '/api/openrouter/models/embedding', body: {}, command: 'get_openrouter_embedding_models', args: undefined },
        { path: '/api/openai/nanogpt/models/embedding', body: {}, command: 'get_nanogpt_embedding_models', args: undefined },
    ];
    ...
    assert.deepEqual(calls, requests.map(({ command, args }) => ({ command, args })));
