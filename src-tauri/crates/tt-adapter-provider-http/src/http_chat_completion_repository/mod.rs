...
impl HttpChatCompletionRepository {
        ...
        F: FnMut(&[u8]) -> Result<(), DomainError>,
    {
        ...
        let consume = Self::consume_sse_response(provider_name, response, framing, |payload| {
            hook(payload)?;
...
mod tests {
    ...
    fn forward_sse_events_keeps_multiline_json_in_one_event() {
        ...
        let event = events.first().expect("multiline event");
        assert_eq!(
            serde_json::from_slice::<Value>(event).expect("valid multiline JSON"),
    ...
    fn forward_sse_events_dispatches_data_lines_without_an_event_boundary() {
        ...
        assert_eq!(events, [b"{\"chunk\":1}".to_vec()]);
        assert!(buffer.is_empty());
    }
