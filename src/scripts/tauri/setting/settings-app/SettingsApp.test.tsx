...
test('Codex auth import is capability-gated and disables its action while pending', async () => {
    ...
    const importButton = within(container).getByRole<HTMLButtonElement>('button', { name: 'Add Codex Auth' });
    ...
    hiddenOptions.viewModel.capabilities.supportsCodexAuthImport = false;
    const { container: hiddenContainer } = mountApp(hiddenOptions);
    expect(within(hiddenContainer).queryByRole('button', { name: 'Add Codex Auth' })).toBeNull();
});
